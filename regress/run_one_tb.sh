...
#
# Exists so regress/run_all.sh --fast can run the suite in parallel. The serial loop
...
#
# Usage: regress/run_one_tb.sh "name|rtl|top" <workdir>
...
if ! SRAM_MODEL=$(cd "$REPO/sim" && "$REPO/regress/sram_model.sh"); then
