#!/usr/bin/env bash
# Mutation-test tb_pe_soc_spi.v: it must FAIL when the properties it claims
...
#
# Usage: regress/mutate_spi_tb.sh
set -u
...
run_case() {
  ...
  (cd sim && iverilog -g2012 -s "$top" -o "$work/$top.vvp" $SRAM_FLAGS \
      ...
      ../tb/tb_pe_soc_spi.v) >"$work/compile.log" 2>&1 || cc=$?
