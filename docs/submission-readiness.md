...
| Host-controller story | **A−** | Bridge is MicroPython-verified (five deployment blockers found and fixed), the R2 read contract is chip-confirmed in simulation for **all 22 of its 22 golden steps** — the four held-at-a-breakpoint ones included, confirmed by the chip's 2026-09-25 re-run, with the real-board run still explicitly unclaimed — and liveness surfaces in the GUI: all recorded, none overclaimed. |
