...
module tb_pe_eth_mac;
  ...
  initial begin
    review_reset;
    review_frame(16'h0806, 46);
    $display("CONTROL type46: valid=%0d bad=%0d len=%0d room=%0d ptr=%0d", nvalid, nbad, last_len, u_mac.room, frame_ptr);
    check(nvalid == 1 && nbad == 0 && last_len == 46 && u_mac.room == 2002 && frame_ptr == 46, "valid type46 control");
    review_reset;
    review_frame(16'h0806, 0);
    $display("RUNT type0: valid=%0d bad=%0d len=%0d room=%0d ptr=%0d folds=%0d crc=%h", nvalid, nbad, last_len, u_mac.room, frame_ptr, folds, crc_state);
    check(nvalid == 0 && nbad == 1 && u_mac.room == 2048 && frame_ptr == 0, "18-byte type runt must be rejected");
    review_reset;
    review_frame(16'h0806, 45);
    $display("RUNT type45: valid=%0d bad=%0d len=%0d room=%0d ptr=%0d folds=%0d crc=%h", nvalid, nbad, last_len, u_mac.room, frame_ptr, folds, crc_state);
    check(nvalid == 0 && nbad == 1 && u_mac.room == 2048 && frame_ptr == 0, "63-byte type runt must be rejected without consuming space");
    for (int extra = 1; extra < 8; extra++) begin
      review_reset;
      partial_frame(extra);
      $display("PARTIAL extra=%0d: valid=%0d bad=%0d len=%0d room=%0d ptr=%0d folds=%0d expected_folds=%0d bit_cnt=%0d crc=%h", extra, nvalid, nbad, last_len, u_mac.room, frame_ptr, folds, 512+extra, u_mac.bit_cnt, crc_state);
      check(nvalid == 0 && nbad == 1 && u_mac.room == 2048 && frame_ptr == 0, "CRC-valid partial byte must be rejected without consuming space");
    end
    if (errors != 0) $fatal(1, "Ethernet structural boundary checks failed: %0d", errors);
    $finish;
  end
endmodule
