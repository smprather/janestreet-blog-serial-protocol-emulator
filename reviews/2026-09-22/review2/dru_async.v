...
module tb_pe_eth_mac;
  ...
  // Independent wire clock; see the sweep parameters below.
  real review_half_ns = 50.0;
  ...
  always @(posedge clk) begin
    if (crc_clr) begin folds = 0; mismatches = 0; end
    if (crc_bit_en) begin
      if (folds < 8*nframe) wantbit = frame[folds/8][folds%8];
      else if (folds < 8*nframe+32) wantbit = fcs[folds-8*nframe];
      else wantbit = 1'bx;
      if (crc_bit_in !== wantbit && mismatches < 8) begin
        mismatches++;
        $display("MISMATCH fold=%0d want=%b got=%b time=%0t state=%0d", folds, wantbit, crc_bit_in, $time, dbg_state);
      end
      folds++;
    end
    if (u_mac.pend && rx_err && dbg_state != 0 && dbg_state != 5 && dbg_state != 6)
      $display("INVALID state=%0d pay=%0d bit=%0d folds=%0d crc=%h time=%0t", dbg_state, u_mac.pay_cnt, u_mac.bit_cnt, folds, crc_state, $time);
    if (frame_bad || frame_valid)
      $display("VERDICT valid=%b bad=%b folds=%0d expected=%0d crc=%h", frame_valid, frame_bad, folds, nframe*8+32, crc_state);
  end
  reg expected_rise [0:200000];
  reg expected_fall [0:200000];
  integer clk_index = 0;
  integer sample_errors = 0;
  always @(negedge clk) expected_fall[clk_index-1] = rx_pin;
  always @(posedge clk) begin
    expected_rise[clk_index] = rx_pin;
    if (clk_index > 20 && sample_errors < 12) begin
      if (u_dru.rx_p_in !== expected_rise[clk_index-3] || u_dru.rx_n_in !== expected_fall[clk_index-3]) begin
        sample_errors++;
        $display("SAMPLE MISMATCH t=%f p=%b want=%b n=%b want=%b", $realtime, u_dru.rx_p_in, expected_rise[clk_index-3], u_dru.rx_n_in, expected_fall[clk_index-3]);
      end
    end
    clk_index++;
  end
  ...
  integer failed = 0;
  initial begin
    for (integer filt = 0; filt <= 0; filt++) begin
      for (integer speed = -1; speed <= -1; speed++) begin
        review_half_ns = 50.0 + speed * 0.005;
        for (integer offset = 0; offset < 1; offset++) begin
          review_reset;
          cfg_filter_en = filt[0];
          @(negedge clk);
          #(offset + 0.2);
          review_frame(16'h0806, 1500);
          trials++;
          if (nvalid !== 1 || nbad !== 0 || last_len !== 1500) begin
            failed++;
            $display("FAIL phase=%0d half=%f filter=%0d valid=%0d bad=%0d len=%0d", offset, review_half_ns, filt, nvalid, nbad, last_len);
          end
        end
      end
    end
    $display("ASYNC SWEEP trials=%0d failures=%0d", trials, failed);
    if (failed != 0 || sample_errors != 0) $fatal(1, "asynchronous DDR capture failed");
    $finish;
  end
endmodule
