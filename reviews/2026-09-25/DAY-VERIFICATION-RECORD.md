...
| **freqmeter TB is RED** | `tb/tb_pe_soc_freqmeter.v` fails, and is **not in `run_all.sh`'s case list** | **fw-timing's WIP, deliberately unwired.** The same-list rule says a protocol not in the list is not in the regression; the act has no firmware yet, so wiring it would break the suite. The merge gate reports the omission as a same-list violation, which is the correct report of a true fact |
