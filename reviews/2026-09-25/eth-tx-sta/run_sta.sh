...
# (wiki/plans/eth-tx-frame-path.md Task 7 Step 2).
#
# Run from the repo root:
#   bash reviews/2026-09-25/eth-tx-sta/run_sta.sh
#
# WHAT IS NEW HERE versus reviews/2026-09-24/serdes-sta/ (Task 5b):
#   * the source list gains rtl/pe_eth_tx.v, so the netlists CONTAIN the
#     frame engine, its 8-byte staging FIFO, the TX-dedicated pe_crc and the
#     32-entry window bank the engine hangs off;
#   * pe_ctrl is NOT screened in this pass: it is unchanged since its own
#     Task-5b screen, and the goal here is the NEW timing classes.
#
...
DIR="$ROOT/reviews/2026-09-25/eth-tx-sta"
cd "$ROOT" || exit 1
