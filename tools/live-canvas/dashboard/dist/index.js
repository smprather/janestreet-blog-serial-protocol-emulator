...
(function () {
  ...
  var FRAME_SCRIPT = [
    ...
    "  // A percent / auto / unit-bearing width is NOT an intrinsic pixel size, but",
    "  // parseFloat('100%') cheerfully returns 100 -- truthy -- so the viewBox",
    "  // fallback below never fires and a mermaid drawing emitted with",
    "  // width=\\\"100%\\\" is believed to be 100 px wide instead of its viewBox",
    "  // width. Every derived scale (fit box, reported fit ratio, the 100% button)",
    "  // is then wrong by that ratio, and the drawing lands in a corner as an",
    "  // unreadable sliver. Mermaid emits width=\\\"100%\\\" on most diagrams, so",
    "  // this was the common case, not an edge case.",
    "  function px(v){",
    "    if(v == null) return 0;",
    "    var m = /^\\\\s*([-+]?[0-9]*\\\\.?[0-9]+)(px)?\\\\s*$/.exec(String(v));",
    "    if(!m) return 0;",
    ...
    "  var iw = px(svg.getAttribute('width'));",
    "  var ih = px(svg.getAttribute('height'));",
    ...
    "  function paneW(){ return wrap.clientWidth || 0; }",
    "  function apply(){",
    ...
    "    if(VIEW.mode === 'fit'){",
    "      var pw = paneW();",
    "      // Not laid out yet. This iframe is created by the pane and React",
    "      // commits its geometry AFTER the srcdoc has parsed, so the first",
    "      // apply can see a zero-width wrap. Sizing to a 1px floor left the",
    "      // drawing as an invisible sliver, and nothing re-measured it -- the",
    "      // stage looked blank until the user clicked Fit. Leaving the SVG",
    "      // alone here is both correct (its own percent width already means",
    "      // the pane width) and temporary: the ResizeObserver below",
    "      // re-applies as soon as the pane has a real size.",
    "      if(pw <= 1) return;",
    "      w = pw; h = ih * (w / iw);",
    "    }",
    ...
    "  }",
    "  function reportFit(){",
    "    if(paneW() <= 1) return;   // unmeasured pane: the ratio is meaningless",
    "    try { parent.postMessage({type:'lc-fit', value: paneW() / iw}, '*'); } catch(e){}",
    "  }",
    "  apply(); reportFit();",
    "  window.addEventListener('resize', function(){ apply(); reportFit(); });",
    "  // The resize listener fires only when the WINDOW changes size. An iframe",
    "  // that is sized by its parent after load does not fire it, so fit would",
    "  // otherwise stay un-applied. Observe the wrap itself.",
    "  if(window.ResizeObserver){",
    "    try { new ResizeObserver(function(){ apply(); reportFit(); }).observe(wrap); } catch(e){}",
    "  }",
