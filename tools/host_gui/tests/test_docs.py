...
class TestDebugActIsHonest(unittest.TestCase):
    ...
    def test_every_demo_beat_named_exists_in_acceptance(self):
        acceptance = (REPO_ROOT / "tools" / "host_bridge"
                      / "acceptance.py").read_text(encoding="utf-8")
    ...
    def test_the_quoted_acceptance_counts_are_current(self):
        ...
        match = re.search(r"RESULT: PASS \((\d+) PASS, (\d+) FAIL, (\d+) SKIP\)",
                          out.stdout)
    ...
    def test_the_model_boundary_is_not_claimed_as_proven(self):
        ...
        self.assertTrue(all(not s["chip_confirmed"] for s in self.steps))

    def test_the_r3_row_is_simulated_not_hardware(self):
        row = [line for line in self.text.splitlines()
               if line.startswith("| Debug control")]
        ...
        board = [line for line in self.text.splitlines()
    ...
    def test_the_opcodes_named_are_the_real_ones(self):
        ...
        for name in ("OP_DEBUG_STEP", "OP_DEBUG_BP_SET", "OP_DEBUG_BP_CLR",
                     "OP_DEBUG_STATUS"):
