    ...
    *,
    ...
    if len(usable) < 4 or usable[-1][0] - usable[0][0] < 20.0:
        return {
            ...
            "samples": len(usable),
            "seconds": round(usable[-1][0] - usable[0][0], 1) if usable else 0.0,
    ...
    ok = growth <= max_growth_mb * 1e6 and object_growth <= max_object_growth
    return {
        "verdict": "bounded" if ok else "leak",
        "samples": len(usable),
...
def _new_stack(*, hostile: bool = False):
    ...
    return SimpleNamespace(
        api=api, session=session, bridge=bridge, clock=clock, ports=ports
    )
...
def run(
    *,
    ...
    reconnect_every: int = 2000,
    assemble_every: int = 500,
    hostile_every: int = 500,
    fuzz_every: int = 250,
    http: bool = False,
    max_growth_mb: float = DEFAULT_MAX_GROWTH_MB,
    max_object_growth: int = DEFAULT_MAX_OBJECT_GROWTH,
    ...
    while done < cycles if cycles else True:
        ...
        try:
            ...
            if done % fuzz_every == 0:
                FZ.run(seed=seed + done, iterations=FUZZ_CHUNK_ITERATIONS, budget_s=10.0)
        ...
        if done % sample_every == 0 or now - last_sample >= sample_seconds:
            objects = gc_object_count() if done % (sample_every * 3) == 0 else None
            samples.append((now - started, done, rss_bytes(), objects))
            last_sample = now
            if progress is not None and len(samples) % 10 == 0:
                progress(done, samples[-1])
    gc.collect()
    samples.append((time.monotonic() - started, done, rss_bytes(), gc_object_count()))
    summary = summarize(
        samples, max_growth_mb=max_growth_mb, max_object_growth=max_object_growth
    )
    return {
        "seed": seed,
        "cycles": done,
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description=(__doc__ or "host soak").split("\n")[0])
    parser.add_argument("--minutes", type=float, default=DEFAULT_MINUTES)
    parser.add_argument(
        "--cycles", type=int, default=0, help="stop after N cycles (0 = minutes only)"
    )
    ...
    parser.add_argument(
        "--max-object-growth", type=int, default=DEFAULT_MAX_OBJECT_GROWTH
    )
    ...
    print(
        f"soak seed={result['seed']} cycles={result['cycles']} "
