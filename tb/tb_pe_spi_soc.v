// tb_pe_spi_soc.v — SPI mode 0 as firmware, end to end on real RTL.
//
...
// `tools/peemu.py`. That is a model written from the same understanding as the
...
// reason — see the note in firmware/spi_xfer.pe and tb/run_firmware_tests.sh.
//
// Program: firmware/spi_xfer.pe, assembled by tools/peasm.py.

`timescale 1ns / 1ps

module tb_pe_spi_soc;
  ...
  // pe_uart_soc (see the header there); this is a TEST FACT about the board,
  ...
  // The shared tick, from rtl/pe_uart_soc.v: TICKS_PER_BIT = CLK_HZ/BAUD/2
  ...
  initial begin
    ...
    if ($test$plusargs("dump")) begin
      $dumpfile("tb_pe_spi_soc.vcd");
      $dumpvars(0, tb_pe_spi_soc);
    end
    ...
    //
    // tb_pe_uart_soc has the same four-clock gap (`repeat (4) @(posedge clk)`
    ...
    // tools/peemu.py agreeing perfectly on dmem, which is what identified the
