...
module formal_pe_soc_refute #(
  ...
  // ---- C2: the missing guard (EXPECTED TO FAIL) -------------------------
  // The RTL guards CLEARING the owner (C1 in formal_pe_soc.v) but SETS it
  // unconditionally: `if (io_wdata[2]) tx_path <= 1'b1;` has no ser_tx_busy
  // term, so a TXCTRL write during a SERDES transmission steals the codec
  // mid-frame. This target states the guard the design would need; a
  // COUNTEREXAMPLE here is the machine-checked form of finding F2. Do not
  // "fix" this wrapper to make it pass -- the wrapper states the requirement,
  // the RTL does not meet it, and the manager owns the ruling.
  always @(*) begin
    if (!p_tx_path && fv_tx_path) assert (!p_ser_busy);
  end
endmodule
