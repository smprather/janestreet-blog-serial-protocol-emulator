...
## [2026-09-25] ruling | the five page rules become a gate, with a pinned baseline
## [2026-09-25] self-inflicted | four bugs the new gate found in its own author, and the assertion that caught the worst one
## [2026-09-25] wire | the host-bus and debug pages, and the host-bus figures
## [2026-09-25] docs (diag-proto) | three chip-host protocol figures, both project maps refreshed, three wiki concept pages
...
- Manager ruling, option (c) pinned baseline plus a (a)-lite taxonomy extension, assigned to this task with `regress/` scoped to this one tool.
- **SCHEMA taxonomy extended from 20 to 43 tags.** The measurement found **23** distinct off-taxonomy tags in use across 9 pages (`boot codec debug ethernet firmware floorplan gui host-controller integration loader observability pads pico pinout planning protocol-emulation serdes spi status timing tx usb wrapper`). Ruled a taxonomy defect, not 23 page defects, so the tags were added rather than the pages rewritten. **A CORRECTION TO THE RECORD: my own earlier index text and the dispatch that quoted it both said 22. The counted number is 23, and one tag (`protocol-emulation`) was dropped in a first draft of the taxonomy section and only came back because the same parse that verifies the list was used to check it.**
- The extended list is formatted so every bullet contains ONLY comma-separated tags, with no prose: an unscoped parse of SCHEMA.md read three words of a parenthetical ("as", "distinct", "surface") as tags. The parser is scoped to the `## Tag Taxonomy` section, and a rename of that heading is a loud harness error rather than an empty taxonomy.
- NEW `regress/check_wiki_pages.sh`: enforces the five rules over the 43 hand-written pages (`raw/` and the four meta pages excluded; the seven GENERATED reference pages are IN scope, since a generator emitting a bad page should be caught here too). The taxonomy is READ FROM SCHEMA.md, not hardcoded, so the list and the gate cannot drift apart.
- NEW `wiki/.known-rule-violations.txt`: the pinned baseline, 12 violations across 8 pages (4 no-frontmatter, 8 zero-outbound-links), each with a one-line reason. The pin is enforced in BOTH directions — a NEW violation is red, and a pinned violation that has been FIXED is also red, because a baseline that silently outlives its defect is a checklist rather than a pin. The shape is borrowed from `tb/r3-vectors/R3_KNOWN_DIVERGENCES.txt`.
- Wired into `regress/run_all.sh`'s documentation section beside the generated-page checks, using the same `stale` accumulator. Deliberately NOT stamped with `chip_dep_stamp`, and the reason is written into the script header rather than left as an omission.
- NEW `regress/test_check_wiki_pages.sh`: 13 cases, all passing, and they are the negative control — a gate only ever seen green is a script that prints reassuring text. Covers a NEW violation of each rule, the STALE direction, closing a pin properly (fix the page AND drop the line), and four fail-closed paths.

- Writing the gate produced four real defects in it, all found before it was ever wired in, and the record matters more than the fixes.
- **The one that mattered was caught by the coverage assertion, not by reading.** The first draft handed awk the page LIST as its single input, so it read one file — the list — the violation set came out empty, the count came out zero, and the gate would have reported a clean wiki. What stopped it was comparing the number of pages the rule pass actually processed against the number it was handed. A SMALLER violation set looks cleaner, not broken, so the count is the only defence. The rule pass is now one awk invocation per page — the same discipline `regress/check_shell_syntax.sh` uses for `bash -n`, and for the same reason.
- A second, related: the first draft used awk's `ENDFILE`, which **mawk does not have** — the default awk on this kind of box would have treated it as an unparseable rule, emitted nothing, and reported a clean wiki. The awk on this host is gawk and would have accepted it, so the portability argument alone did not save this either; the coverage assertion is what would have.
- The `not-in-index` rule took three attempts. A bare substring is satisfied by the TEXT of `plans/pe-ctrl-readback`, so `plans/pe-ctrl` counted as listed without being listed. Two regex versions then matched neither the `[[p]]` nor the `[[p|alias]]` form, and the rule fired on all 43 pages — every one of them wrongly, which is the failure direction that looks most like success. It is now `index()`, a literal substring search with nothing to escape, verified against all four cases including the prefix-collision one before being adopted.
- A fourth: `no-type` fired on the four pages that have no frontmatter at all, reporting one defect as two violations and needing two baseline lines to pin one problem. The rule is now guarded on the page HAVING frontmatter, so the two are independent.
- One more worth recording from the self-test itself: two of its first cases failed for a reason unrelated to the gate — both `sed`s were written against a page's exact current text, both silently matched nothing, and both cases then asserted on a page that had not been broken. So the self-test now REFUSES to continue when a mutation is a no-op, and says so by name. A case that does not set up its own condition is not a test.
- Two smaller ones, both found by the linter rather than by me: `FILENAME` is unset inside an awk `BEGIN` block, so every violation first printed with an empty page field (the visible symptom was 12 correct-looking STALE reports and 4 nameless NEW ones — a gate that fires on the right rule for the wrong reason is still a gate nobody can read); and two lines computing the violation count were lost when a block was replaced, which the linter caught as an unbound variable.

- Wired in two pages from `docs/diag-proto`, read from its PUSHED ref rather than its worktree, and added a **The host bus and the debug contract** subsection to Concepts for them:
  - `concepts/host-chip-protocol` — the frame, the wait-word rule and why it is safe, the opcode table, the 11-word core header R2 stopped stubping, and the read-count ceiling. Explicit that this SPI shares nothing with the firmware master's SPI but the name.
  - `concepts/debug-control` — the four debug states and six numbered traps, led by the one that costs boards the most time (the run strap masked BOTH ways under a hold), and why a host's own model cannot show most of it.
- Wired the three host-bus figure sets into the diagrams note: `proto-spi-framing` (frame word/bit layout and where the wait words sit), `proto-r2-read-path` (four read opcodes, the 11-word header, the count ceiling), `proto-r3-debug-control` (the four states, STOP-BEFORE, the opcode flows, the `BP_SET` subtlety). Also recorded the two refreshed project maps.
- The **In flight** section updated to say what has LANDED rather than only what has not, and to note that `debug-control.md` had uncommitted edits in flight when read, so the copy linked here is the committed one.
- Re-verified every wikilink in all three of this worker's pages against the actual remote branches: **0 unresolved**, with the four forward references resolving once `docs/diag-timing` and `docs/diag-proto` are fetched.

- Three new figures under `diagrams/`, each `.puml` + colocated `.png`/`.svg`, each built from the **RTL response builders** rather than the contract prose: `proto-spi-framing` (request/response word and bit layout, CRC-16/CCITT-FALSE, the WAIT-WORD contract incl. the OE-on driven `0xFFFF` pad and its scope to `0x13`/`0x14`), `proto-r2-read-path` (the four read opcodes, the 11-word header w0..w10, the read-count ceiling and the deliberate `count==0`-is-RANGE), `proto-r3-debug-control` (the four-state machine, STOP-BEFORE on `dbg_next_pc`, the four opcode flows, the masked run strap, and the BP_SET two-path split).
- Both project maps refreshed **on** the already-pushed fw-bus baseline (`diagrams/` imported from `origin/fw-bus-protocols` @ `75b0e3e`, not rebuilt from main) — plan **0.9161**, progress **1.1103**, both inside the Task-5a targets. Task-5a discipline held: zero loss proven by parsing both maps before/after (43->50 components, 45->50 edges, 7->10 notes; 49->56, 55->60, 9->12), with only 3 package TITLES changed and those are sanctioned C3 renames.
- Three accuracy corrections applied **after verifying each against source, not on trust**: C1 the execute gate restated in its repaired `=== 1'b1` form at all four sites with the missing undriven-input convention; C2 `0x24 DEBUG_STATUS` + its 10-word response added to both maps (which previously contradicted the figure on the same branch); C3 the 260-clock tick named beside every conclusion, with port 4 `I2CTICK` (1 us, `I2C_TICKS = 60` exactly) named alongside.
- Wiki: `concepts/host-chip-protocol.md` (158 lines), `concepts/debug-control.md` (196), `concepts/formal-verification.md` (171) and its split companion `concepts/protocol-formal-gaps.md` (160) — all under SCHEMA's ~200-line split threshold, frontmatter 7/7, every wikilink resolving.
- The formal pages keep the denominators straight and say so explicitly: **5 modules**, **10** = the ten `run_target` gate rows (8 PROVED / 1 REACHABLE / 1 VACUOUS; 6 `bmc` + 4 `induct`) in `formal/results/summary.txt`, **11 targets** = those ten plus the SMT unlock, **14 mutants** all CAUGHT (4 `bmc` + 10 `induct`). Every one of those numbers re-derived from the machine-recorded files rather than copied, and the page states plainly that "10" counts *targets*, not assertions — `pe_ctrl` alone carries twelve claims.
- `protocol-formal-gaps.md` carries the four `pe_ctrl` depth-16-only claims and their exact blocker (live register vs lagged snapshot; k-induction starts from an *arbitrary* state, so the antecedent can be met with an unreachable value), the reformulation thread under the mutant-kill bar (C1 **ACCEPTED**, now unbounded and two-engine; C2/C3/C4 **REJECTED** and kept depth-16), the inductive IFG floor and why a cap-kill was read as a strategy signal, and the H3 vacuous-claim removal.
- Finding recorded rather than shipped: the review document's `pe_ctrl.v` line references for C1's antecedent (`:994`/`:1017`/`:1201`/`:1205`) are **stale by ~15 lines** and land on comments. Verified the claim's antecedent independently instead — `r_slot` has exactly four writers, is loaded to `16'd1` at both accepts (`pe_ctrl.v:1009`, `:1032`), incremented in `R_WAIT` (`:1236`, `:1241`), and **nothing assigns it 0** — and the page carries the corrected pointers plus the note that the structure is right and the pointers drifted.
- Finding recorded: the dispatch named a "R3.1 claim-reformulation thread" and `reviews/2026-09-25/DAY-VERIFICATION-RECORD.md`; **neither exists under those names** — the thread is `reviews/2026-09-25/FORMAL-STRENGTHENING.md` §6–7, and the day record was added in `04dc356` on a ref not in this worktree. The substance is present and cited from its real home.
- Progress GREEN/RED set by **measurement**: the brief listed the FREQMETER testbench as the deliberate WIP, but run from `tb/` it is `PASS: all checks`; the genuinely-WIP act is **HC-SR04** (5 failing checks *and* `tb_pe_soc_sr04.v` unwired from `run_all.sh`, so a green suite is silent about it). Both reproducible with one command, and the note also names the false red I nearly shipped (that TB must run from `tb/`, not the repo root).
- Process finding for the fleet: `git push origin <branch>` reported `"Everything up-to-date"` with **rc=0** while the remote was a commit behind and did not refresh the local tracking ref. Only `git push -v origin HEAD:refs/heads/docs/diag-proto` moved it. Verify with `git ls-remote`, and read content out of the pushed ref rather than the working tree.
- Verification: `plantuml --check-syntax` rc=0 on all five diagram sources, `--check-graphviz` OK (PlantUML 1.2026.8 / dot 16.1.0); an audit of a `git archive` extract of the pushed ref found frontmatter 7/7 on all four pages, every wikilink resolving, and 5/5 svg sidecars well-formed under a real XML parser. Two false positives from that audit were traced to **my own instrument** (a `<?xml` grep that PlantUML output can never satisfy, and an audit whose fixture failed to extract) rather than reported as deliverable defects.
- Boundary: `rtl/`, `tb/`, `regress/`, `wiki/index.md` and `wiki/log.md` (other than this entry) are 0-diff from main. `wiki/index.md` still needs entries for the four new pages and the three new figures — that wiring belongs to the wiki-features worker.
## [2026-09-25] pre-merge check | the page-rules gate, run against the merge before the merge
- The gate only ever saw the pages on MY branch. The four sibling pages do not exist here and WILL be checked the moment the branches merge, so the honest question is whether landing this gate turns someone else's suite red at merge time. That is the exact hazard ruling (c) was chosen to avoid, so predicting it was mine to do rather than leave to be discovered by whoever merges.
- Method: not an eyeball count — the real checker, run against a scratch tree built from my wiki plus the sibling pages extracted from their PUSHED refs, and re-run after each change to my own files. Two failures predicted:
  - `concepts/debug-control.md` `zero-outbound-links`, 1 distinct outbound target against a minimum of 2. Confirmed on the pushed ref AND in the author's working copy, so it is live, not a stale read. **This page is not mine; it needs one more wikilink.**
  - `concepts/formal-verification.md` `not-in-index`. **This one WAS mine** — the index is my file, and a page nobody has linked is a page the gate is right to complain about. Fixed by forward-linking it, which the gate then confirmed: the predicted failure count went 2 → 1.
- **A pin in the baseline cannot express a forward expectation, and the reason is worth the ink.** A pin is a statement about a page that EXISTS on the branch holding it. Pre-pinning `debug-control` would have made MY branch red the moment it was written, because the page is absent here and the violation therefore does not occur — the bidirectional STALE check turning a well-meant precaution into a false alarm. Recorded in the baseline header: merge-time violations are PREDICTED AND REPORTED, not pre-pinned, because a pin exists for violations nobody is going to fix (the `plans/` corner) and this one has an author actively editing the file. If the wiki ever needs to express a forward expectation, it wants a third baseline class with its own rule; deliberately not built for a single case.
- Two smaller things found in the same pass, both on the sibling's side and reported rather than fixed: `concepts/formal-verification.md` (still untracked at the time) links to `[[concepts/protocol-formal-gaps]]`, which exists in NO branch and no worktree — a dangling link that will resolve to nothing; and the index line for that page now says on its face that it is not yet pushed, so the one unresolved link in this index is stated rather than left to be found.



## [2026-09-25] docs | host stack concept page added
## [2026-09-25] docs (diag-proto) | the ISA and the SoC wiring, as two architecture concept pages
- Protocol figures for the bus family (pw-diag-bus, `docs/diag-bus`): five sets, `diagrams/proto-i2c-adv.puml`, `proto-spi3-crc`, `proto-uart-flow`, `proto-midi`, `proto-dmx512` — **25 figures, each block rendered to a colocated `.png` AND `.svg`** (1 source + 5 PNG + 5 SVG per protocol), headless per `diagrams/README.md`. Each set is (a) the engine's state machine **as the code has it** (`dmem[10]`'s chain, the two bit engines, the per-cell padding branches), (b) a cell/bit-level frame breakout, and (c) timing figures with real numbers. Every number is one I **measured**: all five Block 2/3 testbenches were compiled and run out-of-tree against this worktree's RTL with the PDK SRAM model, and all five PASS — SCLK period **4.367 µs**; I2C min tLOW **5916 ns** / min tHIGH **5999 ns**, max tLOW **15016 ns** under a 900-clock stretch, 70 polls; CTS released **417.225 µs** with the first start bit **0.31 µs** later and a measured frame period of **86.66 µs**; MIDI cell **31.9987 µs = 31,251 baud**, spread **0.0000 µs** over 14 frames; DMX break **88.06 µs**, mark **12.38 µs**, 513/513 slots, cell **3.9998 µs = 250,010 baud**.
...
## [2026-09-25] gate | the diagram renders get a freshness check, and the gate finds a bug in itself
- Closed the gap pw-diag-bus measured: **nothing gated `diagrams/` at all**, and the directory had gone 26 -> 106 files (22 PlantUML sources, 84 renders). A stale render is the worst kind of documentation defect because it is not wrong-looking: the picture is still a correct picture OF SOMETHING, just not of the source beside it, and a reader has no way to tell. The project maps already carry that history — a hand-drawn map claiming a pin matrix that did not exist.
- NEW `regress/check_diagram_renders.sh`. Deliberately a SEPARATE file rather than a sixth rule in the page gate: that gate's whole virtue is being a dependency-free always-runnable five-rule pass, and folding in a java-and-plantuml dependency would mean it could not report anything useful when plantuml is absent. Same split as the generated-page drift checks, which are a different job from checking a hand-written page against the schema.
- **Measured before built, because a freshness gate red on day one is worse than no gate — it gets routed around and is then worse twice over.** Three things measured, not assumed: the renders are byte-REPRODUCIBLE in both formats when re-rendered with the documented command (had PlantUML embedded a timestamp or version banner, the idea could only ever be red); 22 sources x 2 formats render in ~4 s, noise against a multi-minute regression; and 84 of 84 renders are current, so the gate ships green rather than shipping behind a wall of known failures. Byte comparison, not mtime: a clone rewrites identical content and advances every mtime, so an mtime check would call the whole directory stale after checkout.
- **The self-test found a real bug in the gate, which is the whole reason it exists.** The second direction originally asked "does a .puml exist whose base name matches this render" — which is NOT the same question. A checked-in `proto-midi_009.png`, a numbered panel BEYOND the ones `proto-midi.puml` produces, passed: the produced sweep only walks files the render produced, and the base-name sweep stripped the `_009` and found `proto-midi.puml` happily. So a stale EXTRA panel — exactly what a deleted or renamed panel leaves behind — was invisible forever. Now the second direction is set membership: is this checked-in file one of the files the source produced? That catches the extra panel, a hand-copied render, and a render whose source was deleted or renamed, in one test.
- Two failure paths in the self-test were unreachable as first written, and both are the same mistake in different clothes: an `rm` of a `.puml` that did not exist, and a PATH that broke the tools used *before* the plantuml check, so the script reported the wrong reason. Both now refuse to continue unless the setup actually happened — the same no-op-mutation guard the page-gate cases already had.
- **A design fault in the self-test itself, found because the repository changed under it.** Its green-asserting cases copied the LIVE wiki, so they asserted "green" on whatever state the tree happened to be in; the day another worker's page arrived carrying a rule violation, three of them failed at once and none of them was testing the gate. A self-test whose pass/fail depends on live state is a canary for other people's work. Those cases now run against a synthetic compliant fixture built to clear the checker's own floors; the red-asserting cases still copy the live tree, because extra violations cannot make a red tree green.
- Wired into `run_all.sh` beside the page-rules gate. `tools/host_bridge/acceptance.py` is left alone deliberately — see the entry below.

## [2026-09-25] wire | the bus protocol family, the host stack, and the taxonomy growing to 44
- Wired the five bus protocol pages from `docs/diag-bus` (`i2c-adv`, `spi3-crc`, `uart-flow`, `midi`, `dmx512`), the two input-capture pages that had arrived unmentioned (`dht11`, `ds18b20`), and `concepts/host-stack` from the host-controller-gui merge.
- **The gate found two pages no dispatch had mentioned.** `protocol-dht11` and `protocol-ds18b20` were already on main and unlinked; the page-rules gate reported them as `not-in-index` in the same run that reported the eight the dispatch did name. Worth recording as a mechanism result: the thing that found the unwired pages was the rule, not a person reading a list.
- `concepts/host-stack` moved the section it belongs to: the host-bus subsection is now "The host bus, the debug contract, and the host stack", because the three are the two halves of one conversation and splitting the stack onto its own line made the map less useful, not more.
- **Taxonomy 43 -> 44.** `concepts/host-stack` carries the tag `host`, which was not in the taxonomy. Ruled the same way as the first 23: a tag in active use, obviously useful, and in the same family as `host-controller`, `gui` and `pico` is a taxonomy defect, not a page defect. The gate reported it; the fix was to the schema.
- **Two pins added for pages that arrived on MAIN with the link rule unmet** (`debug-control`, `host-stack`, one link each). Unlike the deferred `plans/` corner, these are not cleanup: without a pin the gate is red on main and blocks every merge behind it. The pins are self-clearing — the moment either author adds a second link the checker turns red and DEMANDS the lines come out — so a pin here is a receipt, not an excuse. Both are recorded with the dispatch that is fixing them.
- Gate state after all of it: 57 hand-written pages, 44 taxonomy tags, 14 pinned violations across 10 pages, **0 new, 0 stale**. Both gates green, the 26-case negative control green, 36 scripts in `regress/` each parse under their own `bash -n`, and every wikilink in this worker's three pages resolves.

## [2026-09-25] FINDING, NOT MINE | `tools/host_bridge/acceptance.py` does not compile on main
- `python3 -m py_compile tools/host_bridge/acceptance.py` fails: **SyntaxError, leading zeros in decimal integer literals**, at line 221. The file does not parse, so it does not import, so the no-board acceptance demo the demo walkthrough and this worker's `concepts/overview` both cite cannot currently run.
- **Root cause, and it is one paragraph, not 666 problems.** A documentation paragraph was spliced into the middle of a function body, immediately after a `return (f"..." f"...")` statement had already closed — so bare English text is being parsed as Python. The linter's 666 findings are the cascade of that single splice.
- **It is not this worker's file and not this worker's change**, verified rather than asserted: `tools/host_bridge/acceptance.py` is byte-identical to `origin/main`, this branch's diff against main for it is empty, the introducing commit is `2868309 docs(host-gui): the flip was generated right and typed wrong in five places`, and the same file **compiles on all three docs branches** — so the breakage is specific to main.
- **Deliberately not fixed here.** The dispatch scopes this task to three wiki pages, then `regress/` for one tool, and explicitly says not to touch other work. `tools/host_bridge/` is the host-controller-gui owner's file and the owner is actively editing that string (the commit subject is about text that was "generated right and typed wrong"). Editing it from a docs branch would risk exactly the class of collision the ownership boundary exists to prevent, and a 666-error linter cascade in a file I did not write is a signal to REPORT, not to take ownership of.
- The fix looks like moving that paragraph into the docstring immediately above it, which already carries three similar paragraphs in the same voice. Routed to the manager for the host-side owner; it is a one-paragraph move, not a redesign.

- Split by content, not by shaving: the dispatch asked for one deep-dive at `wiki/concepts/isa-and-soc.md` under 200 lines, and the honest seam is the subject — the **core** (16-opcode ISA, register discipline, the single-cycle choice and the instruction budget it creates) is now `concepts/isa-and-soc.md` at 91 lines, and the **wiring underneath it** (the port-numbering rule, the port space, the pad map, the SRAM macro rule, synthesis and the mapped timing screen) is `concepts/soc-wiring-and-memory.md` at 133. Re-wrapping prose could not reach the threshold because the page held two subjects; a split keeps both substantial. The split was done by extracting the line range, not by retyping it, so no content was lost or transposed in the move.
- **Numbers re-derived, not borrowed.** `pe_cpu` = **392 cells / 5,070.19 µm²**, and both `pe_imem` configurations = **12 cells / 186.88 µm²** with the macro versus **61,057 cells / 1,300,811.66 µm²** with `FLOP=1` — a **~6,960×** difference — all measured on this tree with the flow and liberty `regress/synth_area.sh` uses (yosys 0.69+post, sg13g2 typ corner). The macro figure counts only wrapper logic because a blackbox contributes no cell area to yosys. `pe_imem` cannot be measured standalone at all (the macro is not in the design), which is why the gate passes the `.bb.v` blackbox; that is now stated rather than left as a surprise.
- The `FLOP` fallback rule is shown to be enforced by the **gate, not a comment**: `synth_area.sh` reports the memory blocks twice (`pe_imem_flop`, `pe_imem_macro`), and `regress/sram_model.sh` fails loudly rather than substituting the flop array when the PDK model is absent.
- The port-numbering rule is quoted from `pe_soc.v` with its actual reason: the ISA has **no shift-LEFT** instruction (`0xA` is SHR), so putting a value at bit *k* would cost two extra instructions per bit to fix up; hence outputs low / inputs high, with the cost paid on **reads**, where the mask is a constant rather than a shift. `PIN_IN_MASK` is the **reset direction**, not a permanent property.
- ⚠ **A number still drifting, recorded not fixed.** The single-cycle budget was 32 clocks/byte and **7.5×** over at 40 MHz; at 60 MHz it is 48 and **5×** (240/48 = 5 exactly). `rtl/pe_eth_mac.v` carries the current 5×, but `rtl/pe_crc.v` and `concepts/factored-hardware-blocks.md` still say 7.5×. Both are outside this worker's ownership, so the pages state which is current and flag the other rather than silently picking one.
- **Two dispatch premises did not match the tree.** There is **no `wiki/concepts/overview.md`** anywhere in the wiki (closest: `competition-overview.md`, `factored-hardware-blocks.md`), and there is **no documented "three implementation styles"** — bit-serial / parallel / hybrid — in the RTL headers, the reviews or the blog fulltext, which mentions PIO/PRU only as inspiration. Rather than invent a taxonomy, the page presents what the sources *do* document and explains why: the hardware/firmware split the instruction budget forces, which is the substantive idea underneath that framing.
- **Gate item from the wiki-pages check satisfied.** `debug-control.md` now carries **3 distinct** outbound wikilinks (SCHEMA requires >= 2). For the record the gate's prediction of *one* was measured against an older state: commit `b7ffeed` had already added `[[concepts/formal-verification]]`, so the count was 2 before this change. The third is the frozen contract review, `[[../reviews/2026-09-25/R3-DEBUG-CONTROL-CONTRACT]]`, verified to resolve.
- Verified before push: all six concept pages under 200 lines, each with >= 2 distinct wikilinks, **15/15 wikilinks resolving**, frontmatter 7/7 on both new pages, and no duplicated H1 left by the split.

## [2026-09-25] wire + fix | four more personas, the budget arithmetic corrected, and a pin that cleared itself
- Wired the pages that arrived on `main` since the last merge: `concepts/isa-and-soc` (the 16 opcodes, why single-cycle, and the budget that choice creates) and `concepts/soc-wiring-and-memory` (the port-numbering rule and why it exists, the port space and pad map, SRAM against the flop fallback, the timing screens, an explicit **Limits** section). Then a further four personas the gate reported as unwired: `protocol-freqmeter`, `protocol-nec-ir`, `protocol-sr04`, `protocol-fm-biphase`. Thirteen protocol deep-dives are now linked.
- **Corrected a stale number in `concepts/factored-hardware-blocks.md`** (manager's item): it claimed the CRC LFSR is "~30 instructions/bit, which is 7.5x over budget at 10BASE-T's 4 clocks/bit". Both halves were pre-turbo figures. At the locked 60 MHz operating point a byte costs **48 clocks, and therefore 48 instructions** (6 clocks/bit x 8), against **240 per byte** for CRC-32 — **5.0x** over budget. Corrected with the cites that carry the arithmetic ([[concepts/ethernet-scope]], which tabulates it, and [[decisions/adr-005-60mhz-turbo]] for the clock), and the page now says why the older 7.5x appears: it was the 40 MHz figure, and 60 MHz is what improved it. A number quoted without the operating point it was measured at is a number that rots.
- **A pinned violation cleared itself, in the real world rather than in a test.** `concepts/debug-control.md` had one outbound wikilink when it landed on main, so it was pinned with a dated reason and a note that the fix was dispatched. The author added the second link, and the gate went red with **STALE** — the baseline entry no longer failing — which is the anti-staleness half doing precisely the job it was built for, several hours after it was written. Pin deleted; the gate went green. That is the whole argument for pinning in one episode: the pin was never an excuse, it was a receipt, and it presented itself for collection.
- The `host-stack` pin remains: still one link, its fix not yet dispatched.
## [2026-09-25] docs (diag-proto) | CORRECTION — two of my own "premise" claims were false, and I broke the shared log doing it

- **Two false claims of mine, withdrawn.** The isa-and-soc page asserted that `wiki/concepts/overview.md` did not exist and that no "three implementation styles" were documented anywhere. **Both are false.** `overview.md` exists (672 lines) and documents the three styles itself — firmware bit-bang, a word engine, and dedicated hardware — with the same 48-instructions-per-byte / ~240-instruction CRC-32 arithmetic. I searched a worktree **91 commits behind main**, generalised the stale result into a claim about the project, and dressed it as diligence. This is the exact error this page had flagged in others, and I committed it myself; the correction is in the page and the styles are now named and linked rather than "not found". The lesson is the one the project keeps paying for, pointed at me: **a local read is not a project claim**, and a missing file is far more often a stale checkout than a missing page.
- **I corrupted `wiki/log.md` and reverted it.** Resolving the merge conflict by union, I sorted **individual lines** by timestamp, which shuffled the continuation lines of main's multi-line entries and left fragments. Caught it by sampling the result: entry-format lines went to zero and bare prose fragments appeared mid-file. Fixed by restoring main's version wholesale (`git checkout origin/main -- wiki/log.md`) — now **byte-identical to main**, verified — and re-appending nothing, because the manager had already merged my earlier entry. A shared append-only log is a **UNION at ENTRY granularity, never at line granularity**, and it should not be hand-merged at all. No worker lost an entry; the loss was structural, and that was mine.
...
- **Diagrams gate hardened after use** (`tools/diag/check_diagrams.sh`). Running the repo's own gates over the new tool, and the tool over the repo, found four more things: (1) the `tools/*.sh` scripts are **not covered by `regress/check_shell_syntax.sh`** — it globs `regress/*.sh`, so its "34 scripts" excludes all 7 shell scripts under `tools/`, the new gate included, and the new gate is likewise outside the 16-harness lock/preflight registry; extending either is another owner's file. (2) The checker read its stem list as `for st in $(expected_stems ...)`, so a source named `my figure.puml` was reported as needing `my.png`, `my.svg`, `figure.png` **and** `figure.svg` — four false failures immediately followed by "every render is fresh and byte-consistent" for a file the loop had never visited. No diagram in this repo has a space in its name, so the class was latent, but a checker that is only correct while nobody names a file the obvious way is correct by luck; the loops now read a line at a time, and the self-test plants the case. (3) The only destructive line in the file, `rm -rf "$sandbox/$1"`, now guards **both halves** of its path with `${var:?}`, so an empty parent is a hard error rather than a path like `/c0`. (4) The self-test's own header described six planted cases when it plants seven, which is the stale-description class this tree keeps catching, found by grepping for the sentence I had just edited rather than trusting that the edit had applied.
