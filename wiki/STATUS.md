...
> on the chip and is chip-confirmed in simulation** (18 of 22 golden steps
...
| R2 read contract (host side) | `tools/host_gui/r2_reads.py` | the seven R2 read obligations as named probes; read-range latches sticky `FAULT_RANGE`, READ payload low-word-first ascending, ISA widths (pc 10, a/x/y 8, insn 16) — **chip-confirmed in simulation** for the read-path steps: `tb_pe_ctrl_r2` passes 18 golden steps byte-exact (chip repo `R2-READ-PATH-REVIEW.md`) | 15 cases, green; package `chip_confirmed: false` — 18 of 22 steps confirmed, the 4 held-core steps pending a chip re-run (`R2-HELD-STATUS-BYTES.md`) |
